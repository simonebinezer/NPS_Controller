...
class EmailTemplateController extends BaseController
{
    ...
    public function sendEmail()
    {
        ...
        if ($this->request->getMethod() == 'post' && !($this->request->getPost("email_id"))) {
            ...
            $errors = [
                ...
                'survey' =>
                [
                    'ValidateRecipientList' => 'Please choose a survey.'
                ]
            ...
            if (!$this->validate($rules, $errors)) {
                ...
                $surveyEncData = $model->where('campaign_id', $postData["survey"])->first();
                $toDecFields = EncryptConstants::$survey;
                $surveyData = $this->modelHelper->DecryptData($surveyEncData, $toDecFields);
                ...
                return redirect()->to(base_url('email/emailtemplate'));
            }
        }
    ...
    public function ScheduleMail($postdata, $userId, $emailList, $tenant)
    {
        $model = new ExternalcontactsModel();
        $condition = ['status' => '1'];
        $contactlist = $this->modelHelper->GetAllData($model, $condition, EncryptConstants::$customer);

        $condition = ['id' => 'in'];
        $filters = ['id' => $emailList];
        $contactlist = ArrayFilter($contactlist, $filters, $condition);
        //->whereIn('email_id', $emailList)
        $contact = [];
        $response = false;
        ...
        for ($i = 0; $i < count($contactlist); $i++) {
            ...
            $template = view("template/email-template-survey", ["userId" => $userId, "postdata" => $postdata, "contactdata" => $contact, "tenantdata" => $tenant]);
        ...
        return "Emails have been successfully scheduled and will be sent out within the next hour.";
    }
    ...
    public function getSurveyAnwser($email_Id, $survey_id, $userid, $tenantid, $quest) //, $randomKey)
    {
        ...
        if ($tenantid > 1) {
            ...
            if ($isSurveyPresent) {
                $isResponse = $this->CheckResponse($externalList['id'], $survey_id);
            }
            $db->close();
        } else {
            $model = new ExternalcontactsModel();
        ...
        if ($isSurveyPresent) {
            if (!$isResponse) {
                ...
                $getSurveyData['userData']['question_id_sel'] = $quest;
    ...
    public function getcollectionsubtenant($customer_id, $survey_id, $userid, $tenantid)
    {
        $model = new TenantModel();
        $tenant = $model->where('tenant_id', $tenantid)->first();
        $dbname = "nps_" . $tenant['tenant_name'];
        ...
        if (count($getSurveycount->getRowArray()) > 0) {
            $toDecFields = EncryptConstants::$survey;
            $getSurvey = $this->modelHelper->DecryptData($getSurveycount->getRowArray(), $toDecFields);
        }
    ...
    public function getcollection($email_Id, $survey_id, $userid, $tenantid)
    {
        ...
        $getEncSurvey = $model->where($multiClause2)->first();
        $toDecFields = EncryptConstants::$survey;
        $getSurvey = $this->modelHelper->DecryptData($getEncSurvey, $toDecFields);
        $model = new QuestionModel();
    ...
    public function createsurveyanswer()
    {
        if ($this->request->getMethod() == 'post') {
            ...
            if (!$this->validate($rules, $errors)) {
                ...
                $isResponse = $this->CheckResponse($getSurveyData['customerId'], $this->request->getPost("surveyid"));
                if (!$isResponse) {
                    $answer = array();
                    ...
                    $data = [
                        ...
                        "customer_id" => $getSurveyData['customerId'],
    ...
    public function getquestionnext()
    {
        ...
        if ($this->request->isAJAX()) {
           ...
           // $modelHelper = new ModelHelper();
            //$tenantAnswerList = [];
            // if (count($tenantAnswerEncList) > 0) {
            //     $toDecFields = EncryptConstants::$answer;
            //     $tenantAnswerList = $modelHelper->DecryptData($tenantAnswerEncList, $toDecFields);
            // }
            $db->close();
    ...
    public function CheckResponse( $customerId, int $campaignId)
    {
        $flag = false;

        $model = new SurveyResponseModel();
        $condition = ['customer_id' => $customerId, 'campaign_id' => $campaignId];
    ...
    public function SearchMails()
    {
        ...
        $encResult = $model->select($selectArray)->where('status', 1)->findAll();

        // ->like('email_id', $like);
        $conditions = ['email_id' => 'like'];
        $filters = ['email_id' => $like];

        $result = $this->modelHelper->DecryptData($encResult, EncryptConstants::$customer);
        $result = ArrayFilter($result, $filters, $conditions);
